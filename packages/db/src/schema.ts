...
export const forms = pgTable(
  'forms',
  {
    ...
    /** Field config — JSONB array of FieldConfigSchema from @formstack/shared. */
    fields: jsonb('fields').$type<unknown[]>().notNull().default(sql`'[]'::jsonb`),
...
export const responses = pgTable(
  'responses',
  {
    ...
    /** JSON array of { fieldId, value } — schema validated at write time. */
    answers: jsonb('answers').$type<Array<{ fieldId: string; value: unknown }>>().notNull(),
    /** Free-form metadata: userAgent, referrer, durationMs, geo... */
...
export const formEvents = pgTable(
  'form_events',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    formId: uuid('form_id').notNull().references(() => forms.id, { onDelete: 'cascade' }),
    /** view | start | field_seen | field_completed | submit | drop */
