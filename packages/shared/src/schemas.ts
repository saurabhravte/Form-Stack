...
export const RegisterInputSchema = z.object({
  ...
  password: PasswordSchema,
  workspaceName: z.string().min(2).max(80),
  workspaceSlug: z.string().min(2).max(40).regex(/^[a-z0-9-]+$/),
});
...
const baseField = {
  ...
  /**
   * Conditional logic — show this field only when the referenced field's
   * answer satisfies the predicate. Evaluated client-side at render time
   * and server-side at submission validation.
   */
  conditional: z
    .object({
...
export const FieldConfigSchema = z.discriminatedUnion('kind', [
  z.object({
    ...
    config: z.object({ placeholder: z.string().optional(), maxLength: z.number().int().positive().default(200) }),
  }),
  z.object({
    ...baseField,
  ...
  // The rest accept any-shape config (picture_choice, signature, payment, scheduler, etc.)
  // so the schema is permissive but the builder UI enforces shape.
  z.object({
    ...baseField,
    kind: z.enum([
      'picture_choice',
...
/** A response answer is always { fieldId, value }. `value` is field-kind-specific. */
