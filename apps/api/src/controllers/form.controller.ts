...
export class FormController extends BaseController {
  ...
  async getPublicBySlug(slug: string) {
    ...
    if (form.status !== "published")
      throw ApiError.notFound("This form is not currently accepting responses");
  ...
  async update(userId: string, dto: UpdateFormDto): Promise<FormRow> {
    ...
    if (dto.fields !== undefined) {
      for (const f of dto.fields) {
        ...
        if (!parsed.success)
          throw ApiError.unprocessable("Invalid field config", parsed.error.flatten());
      }
    ...
    if (dto.settings !== undefined) {
      ...
      if (!parsed.success)
        throw ApiError.unprocessable("Invalid form settings", parsed.error.flatten());
